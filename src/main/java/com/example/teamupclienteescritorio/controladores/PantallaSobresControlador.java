...
public class PantallaSobresControlador implements Initializable {
    ...
    private void cargarSobres() {
        ...
        for (SobreSimplificado sobre : Sesion.getSesion().getSobresTienda()) {
    ...
    public void crearTarjetaSobre(SobreSimplificado sobre) {
        ...
        FlowPane.setMargin(tarjeta, new Insets(10));
        ...
        Label precio = new Label("Precio: " + sobre.getPrecio());


        Button abrir = new Button("Comprar");
        abrir.setUserData(sobre.getNombre());
        abrir.setOnAction(this::comprarSobre);

        tarjeta.getChildren().addAll(imagenSobre, nombre, precio, abrir);
        contenedorMisSobres.getChildren().add(tarjeta);
    }

    private void comprarSobre(ActionEvent event) {
        try {
            ...
            data.put("tipoCosmeticos", "comprarSobre");
    ...
    @FXML
    private void irMisSobres() { // calco funcion tienda
        try {
            ObjectMapper mapper = new ObjectMapper();
